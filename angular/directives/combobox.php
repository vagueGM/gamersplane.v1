<div ng-click="$event.stopPropagation()" class="combobox" ng-class="{ 'autocomplete': usingAutocomplete, 'focused': hasFocus, 'resultsOpen': showDropdown && filterData().length }">
	<input type="text" ng-model="search" ng-change="showDropdown = true" ng-focus="inputFocused()" ng-blur="inputBlurred()" ng-keydown="navigateResults($event)" ng-style="{ 'width': comboWidth + 'px' }">
	<a class="dropdown" ng-click="toggleDropdown($event)" ng-hide="usingAutocomplete"></a>
	<div class="results" ng-class="{ 'hidden': !(showDropdown && filterData().length) }">
